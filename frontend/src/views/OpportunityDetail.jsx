  ...
  return (
    <>
      <button className="btn-ghost btn-sm" onClick={() => nav("opportunities")} style={{ marginBottom: 14 }}>
        <Icon name="arrow" style={{ transform: "rotate(180deg)" }} /> {tt("back_to_opportunities")}
      </button>
      ...
      <div className="grid grid-3" style={{ margin: "24px 0" }}>
        <div className="card stat-card"><div className="stat-label">{tt("budget_range")}</div><div className="stat-value mono">{o.estimatedBudget.display}</div></div>
        <div className="card stat-card"><div className="stat-label">{tt("growth_potential")}</div><div className="stat-value" style={{ color: "var(--green)", fontFamily: "'Fraunces',serif" }}>{o.growthPotential}</div></div>
        <div className="card stat-card"><div className="stat-label">{tt("timeline_stat")}</div><div className="stat-value" style={{ fontSize: 17 }}>{o.roadmap?.estimatedTimeline || "4–8 weeks"}</div></div>
      </div>
      ...
      <div className="grid grid-2">
        <div className="card" style={{ padding: 26 }}>
          ...
          <div className="section-label"><Icon name="chart" /> {tt("demand_signal_label")}</div>
          <p style={{ fontSize: 14, color: "var(--ink-soft)" }}>{o.demandSignal} <span className="tag-note"><Icon name="alert" />{tt("ai_informed_estimate")}</span></p>
        </div>
        <div className="card" style={{ padding: 26 }}>
          ...
          <div className="hr" style={{ margin: "18px 0" }}></div>
          <div className="section-label" style={{ color: "var(--green)" }}><Icon name="leaf" /> {tt("future_advice_label")}</div>
          <p style={{ fontSize: 14, color: "var(--ink)" }}>{o.futureAdvice}</p>
        </div>
      </div>

      <div className="card" style={{ padding: 26, marginTop: 20 }}>
        <div className="section-label"><Icon name="bank" /> {tt("potentially_relevant_gov_support")}</div>
      ...
      <div className="card-actions" style={{ marginTop: 24 }}>
        <button className="btn btn-primary" onClick={() => nav("roadmap")}>{tt("see_roadmap")} <Icon name="arrow" /></button>
        <button className="btn btn-secondary" onClick={() => toggleSave(o.id)}>{isSaved ? tt("saved_check") : tt("save_opportunity")}</button>
        <button className="btn-ghost" onClick={() => nav("myReport")}>{tt("view_full_report")} <Icon name="arrow" /></button>
      </div>
    </>
  );
}
